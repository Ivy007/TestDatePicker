 ...
 */
typedef void (^RMDateSelectionBlock)(RMDatePickerController *vc, NSDate *aDate);
...
/**
  ...
  */
typedef void (^RMDateCancelBlock)(RMDatePickerController *vc);

/**
 *  These methods are used to inform the [delegate]([RMDatePickerController delegate]) of an instance of RMDatePickerController about the status of the date selection view controller.
 */
@protocol RMDatePickerControllerDelegate <NSObject>
 ...
 *  返回NSDate对象
 *
 *  @param vc    datePiker控制器
 *  @param aDate 获取到的时间
 */
- (void)dateSelectionViewController:(RMDatePickerController *)vc didSelectDate:(NSDate *)aDate;
/**
 *  返回解析后的时间字符串
 *
 *  @param vc         datePiker控制器
 *  @param dateString 获取到的时间字符串
 *  @param dateFormat 解析时间用到的
 */
...
/**
 This method is called when the user selects the cancel button or taps the darkened background (if the property [backgroundTapsDisabled]([RMDatePickerController backgroundTapsDisabled]) of RMDatePickerController returns NO).
 ...
 */
- (void)dateSelectionViewControllerDidCancel:(RMDatePickerController *)vc;
 ...
 */
- (void)dateSelectionViewControllerNowButtonPressed:(RMDatePickerController *)vc;

@end

/**
 *  RMDatePickerController is an iOS control for selecting a date using UIDatePicker in a UIActionSheet like fashon. When a RMDatePickerController is shown the user gets the opportunity to select a date using a UIDatePicker.
 *
 *  RMDatePickerController supports bouncing effects when animating the date selection view controller. In addition, motion effects are supported while showing the date selection view controller. Both effects can be disabled by using the properties called disableBouncingWhenShowing and disableMotionEffects.
 *
 *  On iOS 8 and later Apple opened up their API for blurring the background of UIViews. RMDatePickerController makes use of this API. The type of the blur effect can be changed by using the blurEffectStyle property. If you want to disable the blur effect you can do so by using the disableBlurEffects property.
 *
 *  @warning RMDatePickerController is not designed to be reused. Each time you want to display a RMDatePickerController a new instance should be created. If you want to set a specific date before displaying, you can do so by using the datePicker property.
 */
@interface RMDatePickerController : UIViewController

/// @name Getting an Instance

/**
 *  This returns a new instance of RMDatePickerController.
 *
 *  @warning Always use this class method to get an instance. Do not initialize an instance yourself.
 *
 *  @return  Returns a new instance of RMDatePickerController
 */
 ...
 *
 *  The delegate must conform to the RMDatePickerControllerDelegate protocol.
 */
@property (nonatomic,weak) id<RMDatePickerControllerDelegate> delegate;
 ...
 *
 *  @warning If you want to change this property you must do this before showing the RMDatePickerController or otherwise setting this property has no effect.
 */
...
/**
 *  This shows the date selection view controller on top of every other view controller using a new UIWindow. The RMDatePickerController will be added as a child view controller of the UIWindows root view controller. The background of the root view controller is used to darken the views behind the RMDatePickerController.
 *
 *  This method is the preferred method for showing a RMDatePickerController on iPhones and iPads. Nevertheless, there are situations where this method is not sufficient on iPads. An example for this is that the RMDatePickerController shall be shown within an UIPopover. This can be achieved by using [showFromViewController:]([RMDatePickerController showFromViewController:]).
 *
 ...
 *
 *  This method is the preferred method for showing a RMDatePickerController on iPhones and iPads when a block based API is preferred. Nevertheless, there are situations where this method is not sufficient on iPads. An example for this is that the RMDatePickerController shall be shown within an UIPopover. This can be achieved by using [showFromViewController:withSelectionHandler:andCancelHandler:]([RMDatePickerController showFromViewController:withSelectionHandler:andCancelHandler:]).
 *
 ...
 *
 *  @warning This method should only be used on iPads in situations where [show]([RMDatePickerController show]) is not sufficient (for example, when the RMDatePickerController shoud be shown within an UIPopover). If [show]([RMDatePickerController show]) is sufficient, please use it!
 *
 *  @warning Make sure the delegate property is assigned. Otherwise you will not get any calls when a date is selected or the selection has been canceled.
 *
 *  @param aViewController The parent view controller of the RMDatePickerController.
 */
 ...
 *  After a date has been selected the selection block will be called. If the user choses to cancel the selection, the cancel block will be called. If you assigned a delegate the corresponding methods will be called, too.
 *
 *  @warning This method should only be used on iPads in situations where [showWithSelectionHandler:andCancelHandler:]([RMDatePickerController showWithSelectionHandler:andCancelHandler:]) is not sufficient (for example, when the RMDatePickerController shoud be shown within an UIPopover). If [showWithSelectionHandler:andCancelHandler:]([RMDatePickerController showWithSelectionHandler:andCancelHandler:]) is sufficient, please use it!
 *
 *  @param aViewController The parent view controller of the RMDatePickerController.
 *  @param selectionBlock  The block to call when the user selects a date.
 *  @param cancelBlock     The block to call when the user cancels the selection.
 ...
 *  @warning Make sure the delegate property is assigned. Otherwise you will not get any calls when a date is selected or the selection has been canceled.
 *
 *  @warning This method should only be used on iPads. On iPhones please use [show]([RMDatePickerController show]) or [showWithSelectionHandler:andCancelHandler:]([RMDatePickerController showWithSelectionHandler:andCancelHandler:]) instead.
 *
 ...
 *  @param formatString 解析格式 默认为yyyy-MM-dd HH:mm
 */
